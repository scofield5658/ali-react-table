...
import { BaseTableCSSVariables, Classes, LOCK_SHADOW_PADDING, StyledArtTableLoadMoreOutterWrapper, StyledArtTableLoadMoreWrapper, StyledArtTableWrapper } from './styles'
import {
  getScrollbarSize,
  OVERSCAN_SIZE,
  shallowEqual,
...
export interface BaseTableProps {
  ...
  /** 水平方向虚拟滚动阈值 */
  hozVirtualThreshold?: number;
  /** 垂直方向虚拟滚动阈值 */
  verVirtualThreshold?: number;
  ...
  /** 继承上层组件高度 */
  useOutterHeight?: boolean

  /** 是否开启无限滚动 */
  useInfiniteScroll?: boolean;
  /** 无限滚动开启后是否展示"加载更多" */
  hasMore?: boolean;
  /** 无限滚动开启后展示"加载更多"的文案组件 */
  hasMoreContent?: ReactNode | string;
  /** 无限滚动开启后展示"加载更多"的注入样式 */
  hasMoreOverlayClassName?: string;
  /** 无限滚动开启后"加载更多"的回调事件 */
  onNextScroll?: Function;
  /** 无限滚动开启后是否展示"已展示全部" */
  hasScrollEnd?: boolean;
  /** 无限滚动开启后展示"已显示全部"的文案组件 */
  endContent?: ReactNode | string;
  /** 无限滚动开启后展示"已显示全部"的注入样式 */
  endOverlayClassName?: string;
  /** 无限滚动开启后"已显示全部"的回调事件 */
  onEndScrollClick?: Function;

  /** 表格是否在加载中 */
  isLoading?: boolean
  /** 数据为空时，单元格的高度 */
  emptyCellHeight?: number | string
...
export class BaseTable extends React.Component<BaseTableProps, BaseTableState> {
  ...
  private updateStickyScroll() {
    ...
    if (!artTable || !stickyScroll || !stickyScrollItem) {
      return
    }
    
    // 验证stickyScroll和artTable是否属于同一个表格实例
    // stickyScroll应该在artTable的同一个父元素（art-loading-content-wrapper）下
    const artTableParent = artTable.parentElement
    if (!artTableParent || !artTableParent.contains(stickyScroll)) {
      return
    }
    
    const tableBodyHtmlTable = this.domHelper.getTableBodyHtmlTable()
    if (!tableBodyHtmlTable) {
      return
    }

    // 验证获取到的 table 元素是否合理
    // 检查 table 是否在正确的 DOM 结构中
    const tableBody = this.domHelper.tableBody
    if (!tableBody || tableBodyHtmlTable.parentElement !== tableBody) {
      return
    }
    ...
    // 如果table宽度为0，说明DOM可能还没有完全渲染，延迟执行一次
    if (innerTableWidth === 0) {
    ...
    if (this.props.useOutterHeight) {
      artTable.style.height = this.props.dataSource.length ? 'unset' : `calc(100% - ${stickyScrollHeight + 1}px)`
    }
  ...
  render() {
    ...
    const artTableWrapperClassName = cx(
      ...
      {
        [dataSource.length ? 'use-outer-border': 'use-outer-border-empty']: useOuterBorder,
        'use-outer-height': useOutterHeight,
    ...
    if (useInfiniteScroll) {
      return (
        <StyledArtTableLoadMoreOutterWrapper className={Classes.artTableLoadMoreOutterWrapper}>
          <StyledArtTableWrapper {...artTableWrapperProps}>
            ...
            >
              <div className={Classes.artTable} id={containerId}>
          ...
          {hasMore ? (
            <StyledArtTableLoadMoreWrapper className={`${Classes.artTableLoadMoreWrapper} ${hasMoreOverlayClassName || ''}`} onClick={() => onNextScroll && onNextScroll()}>
              <div className={Classes.artTableLoadMoreContent}>
                {hasMoreContent || '加载更多'}
              </div>
            </StyledArtTableLoadMoreWrapper>
          ) : hasScrollEnd ? (
            <StyledArtTableLoadMoreWrapper className={`${Classes.artTableEndWrapper} ${endOverlayClassName || ''}`} onClick={() => onEndScrollClick && onEndScrollClick()}>
              <div className={Classes.artTableEndContent}>
                {endContent || '已显示全部'}
              </div>
            </StyledArtTableLoadMoreWrapper>
          ) : null}
        </StyledArtTableLoadMoreOutterWrapper>
      )
    }
  ...
  private initSubscriptions() {
    const { artTableWrapper, tableHeader, tableBody, tableFooter, stickyScroll } = this.domHelper
    ...
    if (useInfiniteScroll) {
      this.rootSubscription.add(
        subscribeScrollEvent(artTableWrapper, (scrollTop: number, scrollHeight: number, clientHeight: number) => {
          // 存在滚动区域
          if (clientHeight < scrollHeight) {
            // 距离底部还剩10px时触发onNextScroll
