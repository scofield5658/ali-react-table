...
/** 订阅指定容器的滚动事件 */
export function subscribeScrollEvent(element: HTMLElement, callback: (scrollTop: number, scrollHeight: number, clientHeight: number) => void): Subscription {
  const subscription = new Subscription()
  const listener = () => {
    callback(element.scrollTop, element.scrollHeight, element.clientHeight)
  }
  element.addEventListener('scroll', listener, { passive: true })
  subscription.add(() => element.removeEventListener('scroll', listener))
