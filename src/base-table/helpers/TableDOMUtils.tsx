...
export class TableDOMHelper {
  ...
  constructor(artTableWrapper: HTMLDivElement) {
    ...
    // 通过 art-loading-content-wrapper 来精确定位，避免匹配到嵌套表格中的元素
    const loadingContentWrapper = artTableWrapper.querySelector<HTMLDivElement>(`:scope > .art-loading-wrapper > .art-loading-content-wrapper`)
    
    if (loadingContentWrapper) {
      // art-table 和 stickyScroll 都在 art-loading-content-wrapper 内，且是兄弟元素
      this.artTable = loadingContentWrapper.querySelector<HTMLDivElement>(`:scope > .${Classes.artTable}`)
      this.stickyScroll = loadingContentWrapper.querySelector<HTMLDivElement>(`:scope > .${Classes.artTable} + .${Classes.stickyScroll}`)
    } else {
      // 回退方案：使用原来的方式
      this.artTable = artTableWrapper.querySelector<HTMLDivElement>(`:scope .${Classes.artTable}`)
      this.stickyScroll = artTableWrapper.querySelector<HTMLDivElement>(`:scope .${Classes.artTable} + .${Classes.stickyScroll}`)
    }
    ...
    this.stickyScrollItem = this.stickyScroll?.querySelector(`:scope > .${Classes.stickyScrollItem}`)
  }
  ...
  getTableBodyHtmlTable(): HTMLTableElement | null {
    if (!this.tableBody) {
      return null
    }
    // 直接遍历 tableBody 的直接子元素，查找第一个 table 元素
    // 这样可以避免 querySelector 可能匹配到嵌套表格中的 table 元素
    for (let i = 0; i < this.tableBody.children.length; i++) {
      const child = this.tableBody.children[i]
      // 只匹配直接子元素，且必须是 TABLE 标签
      if (child.tagName === 'TABLE' && child.parentElement === this.tableBody) {
        return child as HTMLTableElement
      }
    }
    return null
  }
